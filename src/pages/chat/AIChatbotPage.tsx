...
export const AIChatbotPage = () => {
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const elderMsg: Message = { id: (Date.now() + 1).toString(), role: 'elder', text: answer };
      setMessages(prev => [...prev, elderMsg]);
    } catch {
      // Fallback: try direct Gemini without RAG context
      try {
        const answer = await generateRAGResponse(question, []);
        const elderMsg: Message = { id: (Date.now() + 1).toString(), role: 'elder', text: answer };
        setMessages(prev => [...prev, elderMsg]);
      } catch {
        const elderMsg: Message = { id: (Date.now() + 1).toString(), role: 'elder', text: 'Forgive me, young one. The spirits are quiet today. Please try again later.' };
        setMessages(prev => [...prev, elderMsg]);
      }
    } finally {
      setIsTyping(false);
    ...
    <div className="flex flex-col h-[calc(100vh-4rem)] max-w-4xl mx-auto p-4 md:p-6">
      <div className="flex items-center gap-4 mb-6">
        <div className="h-16 w-16 rounded-full bg-earth-200 dark:bg-earth-800 flex items-center justify-center border-2 border-earth-300 dark:border-earth-700 overflow-hidden shadow-inner">
          <Sparkles className="h-8 w-8 text-forest-600 dark:text-forest-400" />
        </div>
        <div>
      ...
      <Card className="flex-1 flex flex-col p-0 overflow-hidden bg-white/50 dark:bg-earth-900/20 backdrop-blur-sm border-earth-200/50 dark:border-earth-800/50">
          ...
          <AnimatePresence>
            {messages.map((msg) => (
              <motion.div key={msg.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[80%] md:max-w-[70%] rounded-2xl p-4 ${msg.role === 'user' ? 'bg-forest-600 text-white rounded-br-sm shadow-md' : 'bg-earth-100 dark:bg-earth-800 text-earth-900 dark:text-earth-100 rounded-bl-sm shadow-sm'}`}>
        ...
        <div className="p-4 border-t border-earth-200 dark:border-earth-800 bg-white dark:bg-earth-950">
          {messages.length === 1 && (
            <div className="flex flex-wrap gap-2 mb-4">
              {SUGGESTED_PROMPTS.map((prompt, i) => (
                <button key={i} onClick={() => setInput(prompt)} className="text-xs md:text-sm px-3 py-1.5 rounded-full border border-earth-200 dark:border-earth-700 bg-earth-50 hover:bg-earth-100 dark:bg-earth-900 dark:hover:bg-earth-800 text-earth-700 dark:text-earth-300 transition-colors">
                  {prompt}
                </button>
              ))}
            </div>
          ...
          <form onSubmit={handleSend} className="flex gap-2">
            ...
            <input type="text" value={input} onChange={(e) => setInput(e.target.value)} placeholder="Ask the Elder Guide..." className="flex-1 bg-earth-50 dark:bg-earth-900 border border-earth-200 dark:border-earth-700 rounded-xl px-4 py-3 outline-none focus:border-forest-500 focus:ring-1 focus:ring-forest-500 text-earth-900 dark:text-earth-100 transition-all" />
            <button type="submit" disabled={!input.trim() || isTyping} className="p-3 bg-forest-600 text-white rounded-xl hover:bg-forest-700 disabled:opacity-50 disabled:hover:bg-forest-600 transition-colors">
